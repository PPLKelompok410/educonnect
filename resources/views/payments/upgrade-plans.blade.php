...
<body class="bg-gray-50">
    ...
    <nav class="bg-white shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @php
                $activeSubscription = \App\Models\Transaction::where('user_id', session('user')->id)
                    ->with('payment')
                    ->latest()
                    ->first();
            @endphp
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 mb-8 flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold font-opensans text-gray-900">
                            Paket Aktif: {{ $activeSubscription->payment->package }}
                        </h3>
                        <p class="text-sm text-gray-600 font-poppins">
                            Diperbarui pada {{ $activeSubscription->created_at->format('d M Y') }}
                        </p>
                    </div>
                </div>
                <form action="{{ route('subscription.cancel') }}" method="POST" 
                    ...
                    @method('DELETE')
                    <button type="submit" 
                            class="px-4 py-2 text-sm font-medium text-red-600 hover:text-red-700 border border-red-600 rounded-lg hover:bg-red-50 transition-colors duration-200">
                        Batalkan Langganan
                    </button>
                </form>
            </div>
            ...
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center">
                    <!-- Back Button -->
                    <a href="{{ route('dashboard') }}" class="flex items-center text-gray-600 hover:text-primary transition-colors">
                        <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                        </svg>
                        <span class="font-poppins">Kembali ke Dashboard</span>
                    </a>
                </div>
                ...
                <div class="flex items-center">
                    <div class="ml-4 flex items-center md:ml-6">
                        <!-- Profile -->
                        <div class="relative">
                            <button class="flex items-center space-x-3 focus:outline-none" onclick="window.location.href='{{ route('profiles.index') }}'">
                                <span class="font-poppins text-gray-700">{{ session('user')->full_name }}</span>
    ...
    <script>
        function selectPlan(planId) {
            ...
            // For example, redirect to payment page or show payment modal
